...
- **PR summaries**: Generate a markdown PR summary from the current branch diff with `:Pitaco summary`, opened in a modal and copied to the clipboard.
...
Types are: build, chore, ci, docs, feat, fix, perf, refactor, revert, style or test.
        ...
        summary = {
            ...
            model_id = "gpt-5-mini",
            additional_instruction = [[
Keep the summary concise and ready to paste into GitHub.
]],
        },
        -- Feature scopes can override ollama request options too
        -- summary = {
        --     provider = "ollama",
        --     model_id = "llama3.1",
        --     ollama_options = {
...
- Each feature may set `provider`, `model_id`, `additional_instruction`, and `ollama_options`.
- If a feature changes `provider` without setting `model_id`, Pitaco falls back to that provider's built-in default model.
- Built-in scoped features include `review`, `commit`, and `summary`.
...
require("pitaco").setup({
    ...
    features = {
        ...
        summary = {
            provider = "openai",
            model_id = "gpt-5-mini",
            additional_instruction = "Keep it crisp and PR-ready.",
        },
    },
})
```

With the example above, review requests use `num_ctx = 16384` while other Ollama-backed scopes inherit `num_ctx = 8192`.

Flat aliases such as `review_provider`, `review_model_id`, `commit_provider`, `commit_model_id`, `summary_provider`, and `summary_model_id` are also accepted, but `features = { ... }` is the preferred format going forward.
